...
public abstract class TestConstructHelper {
    ...
    public static ITile tile(final String tile) {
        if (tile.length() == 3) {
            if (tile.charAt(1) == 'b') {
                return new TileSuitBamboo(TestConstructHelper.n(tile, 2), TestConstructHelper.n(tile, 0));
    ...
    public static IPlayer constructChainedPlayers() {
        ...
        final IPlayer playerN = Player.instanceOf("P2", Wind.NORTH, null, null);
        final IPlayer playerW = Player.instanceOf("P3", Wind.WEST, null, null);
        final IPlayer playerS = Player.instanceOf("P2", Wind.SOUTH, null, null);
