...
public class RoundTest {
    ...
    public void handlePlayer1Tile() {
        ...
        final ITile discardedTile = round.getCurrentPlayer().getTiles().getConcealedHand()
                .iterator().next();

        round.handlePlayer(round.getCurrentPlayer(), Action.DISCARD_TILE,
                discardedTile);

        // assert that treatment has been performed
        assertFalse(round.getCurrentPlayer().getTiles().getConcealedHand().contains(
                discardedTile));
    ...
    public void handlePlayerSeveralTiles() {
        final IRound round = TestConstructHelper.constructSpecialKongGame()
                .getLastRound();
        round
                .handlePlayer(round.getCurrentPlayer(),
                        Action.CONCEALED_KONG, TestConstructHelper
                                .kong(1));

        // assert that treatment has been performed
        assertFalse(CollectionUtils.containsAny(round.getCurrentPlayer()
                .getTiles().getConcealedHand(), TestConstructHelper.kong(1)));
    }
    ...
    private static void assertTurnEnd(final IPlayer lastCurrent,
                                      final IPlayer newCurrent, final IRound round) {
